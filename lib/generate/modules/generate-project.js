...
class GenerateProject {
   ...
   */
  make () {
    console.log('\x1b[30m\x1b[42m%s\x1b[0m', '[make]')
    this.templateObj = new Template(['g', 'module', 'exemple', 'exemple'])
    return this.makeProject()
  }
  makeProject () {
    console.log('\x1b[30m\x1b[42m%s\x1b[0m', '[makeProject]')
    Promise.all(this.urls.map(url => this.templateObj.createDirectoriesRecurcive({directoryURL: url})))
      .then(success => {
        ...
        this.exec()
        const TpExemple = new TemplateExemple(__base)
        TpExemple.exec().then(() => {
          TpExemple.init()
        })
      }, error => {
...
`
  ...
  async exec () {
    ...
    if (stderr) {
      console.log('\x1b[30m\x1b[41m%s\x1b[0m', '[ stderr npm install ]')
      console.log('\x1b[31m%s\x1b[0m', stderr)
    }
    console.log('\x1b[30m\x1b[42m%s\x1b[0m', '[ stdout  npm install ]')
